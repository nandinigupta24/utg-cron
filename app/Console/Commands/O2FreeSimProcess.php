...
class O2FreeSimProcess extends Command {
     ...
     * @var string
     */
    protected $signature = 'command:O2FreeSimProcess';
...
//        $date = date('Ymd', strtotime('-1 day'));
        ...
        if (ssh2_auth_password($connection, $serverUser, $serverPassword)) {
            ...
            $sftp_fd = intval($sftp);
            $files = scandir("ssh2.sftp://" . $sftp_fd . "/./", SCANDIR_SORT_DESCENDING);
            $newFile = [];
            $Count = 0;
            foreach ($files as $value) {
                if (strpos($value, 'outboundcalling [' . $date) !== false) {
                    /* START */
                    $Count++;
                    if ($Count == 1) {
                        continue;
                    }
                    $SaveFileName = date('Y-m-d') . '-' . $value;
                    $O2FreeSimFileImport = new O2FreeSimFileImport();
                    $O2FreeSimFileImport->file_name = $value;
                    $O2FreeSimFileImport->save_file_name = $SaveFileName;
                    if ($O2FreeSimFileImport->save()) {
                        $UpdatedID = $O2FreeSimFileImport->id;
                    }

                    $content = file_get_contents("ssh2.sftp://" . $sftp_fd . "//" . $value);
                    $fp = fopen("/var/www/html/cron/storage/Automation/O2FreeSim/In/" . $SaveFileName, "w");
                    fwrite($fp, $content);
                    fclose($fp);
//                    $newFile[$SaveFileName] = $value;
...
//         $FILEname = '/var/www/html/cron/storage/Automation/O2FreeSim/In/2019-02-02-outboundcalling [20190201 141658].csv';
...
//            }
            ...
            if($value['packdesc'] == 'Big Bundle £5'){
                $listId = 30031;
            }else{
                $listId = 3002;
            }
            ...
            $postData[$key]['source_code'] = 'O2_FREESIM_RD-' . date('Ymd') . '-' . date('His');
            ...
            $postData[$key]['custom_fields'] = ['O2Template' => @$value['template'],
                ...
                'AboutCust3b' => iconv("Windows-1252", "UTF-8",@$value['packdesc']),
        ...
        $O2FreeSimFileImport->number_of_records = $TotalArray['number_of_records'];
        $O2FreeSimFileImport->loaded_records = $TotalArray['loaded_records'];
        $O2FreeSimFileImport->duplicate_on_loaded = $TotalArray['duplicate_on_loaded'];
        $O2FreeSimFileImport->count_of_PACKDESC = count($TotalArray['count_of_PACKDESC']);
        $O2FreeSimFileImport->count_of_FILECODE = count($TotalArray['count_of_FILECODE']);
