...
class AutomationMTA extends Command {
     ...
     * @var string
     */
    protected $signature = 'command:AutomationMTA';
    ...
    public function handle() {
        ...
        $listId = 3038;
        $campaignId = 3038;
        $localServerPath = storage_path('Automation/MTA/');
...
//        $fileImportArray = FileImportLog::where('type', 'MTA')->where('created_at', 'like', $dateWorking . '%')->pluck('filename')->toArray();
        $fileImportArray = FileImportLog::where('type', 'MTA')->where('created_at', 'like', '2019-06-19%')->pluck('filename')->toArray();
        ...
        if (ssh2_auth_password($connection, $serverUser, $serverPassword)) {
            ...
            $files = scandir("ssh2.sftp://" . $sftp_fd . "/home/MTAAmin/", SCANDIR_SORT_DESCENDING);
            $newFile = [];
            foreach ($files as $keyval => $value) {
                ...
                $date = date("Y-m-d", filemtime("ssh2.sftp://" . $sftp_fd . "/home/MTAAmin/" . $value));
                if ($date != $dateWorking) {
                    continue;
                }
                ...
                $FileImportLog->type = 'MTA';
                $SaveFileName = 'MTA-' . date('Y-m-d') . '-' . $keyval . '-' . $value;
                $content = file_get_contents("ssh2.sftp://$sftp_fd/home/MTAAmin/" . $value);
        ...
        $arrayMailTo = ['dialerteam@usethegeeks.zendesk.com'];
        ...
        $mail_data['subject'] = !empty($data['subject']) ? $data['subject'] : 'MTA Leads - ' . date('Y-m-d');
