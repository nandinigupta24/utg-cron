...
class OptinAll extends Command {
     ...
     * @var string
     */
    protected $signature = 'command:OptinAll';
...
//                             ->select('saleid','title','first_name','last_name','address1','address2','address3','city','postal_code','campaign_name','email','agentid','createddate')
        ...
        /* START O2 PREMIUM */
        $PremiumData = DB::connection('MainDialer')->select("Select s.sale_id,c.lead_id, c.title, c.first_name, c.last_name, c.phone_number, c.email, c.address1, c.address2, c.city, c.postal_code,c.teamname,c.current_supplier, c.happy_with_supplier, c.in_contract,c.bb_switch_call,u.full_name,s.campaign_sold_on,s.sale_date,u.user,c.DateSent
                    from O2Script.customers c
                    INNER JOIN O2Script.sales s ON c.lead_id = s.lead_id
                    JOIN custom_view.list l on l.lead_id=c.lead_id
                    join custom_view.users u on c.fullname = u.full_name
                    where c.DateSent BETWEEN '".$start." 00:00:00' AND '".$end." 23:59:59' AND c.bb_switch_call = 'yes'
                    and s.campaign_sold_on = '3003'
                    group By c.lead_id");
        ...
        if (!empty($PremiumData) && count($PremiumData)) {
            foreach ($PremiumData as $key => $value) {

                $Count++;
                $dataCheck['PREM_OPTIN']['total'] ++;
                $uniCode = "PREM_OPTIN-" . $value->sale_id;
                ...
                $Optins->token = '';
                $Optins->custom_fields_AdentID = @$value->user;
                $Optins->campaign_name = 3003;
                $Optins->UniqueCode = $uniCode;
                $Optins->custom_fields_optindate = $value->DateSent;
                $Optins->duplicate_status = $duplicate;
                if ($Optins->save()) {
                    if ($duplicate == 'no') {
                        $postData[$Count]['import_id'] = $Optins->sale_id;
                        $postData[$Count]['data_list'] = $listId;
                        $postData[$Count]['main_phone'] = @$value->phone_number;
                        $postData[$Count]['title'] = get_empty(@$value->title, '');
                        $postData[$Count]['first_name'] = get_empty(@$value->first_name, '');
                        ...
                        $postData[$Count]['city'] = get_empty(@$value->city, '');
                        $postData[$Count]['email'] = get_empty(@$value->email, 'test@gmail.com');
                        $postData[$Count]['source_code'] = 'PremOptin-' . date('Ymd') . '-' . date('His');
                        $postData[$Count]['source'] = $SourceID;
                        $postData[$Count]['custom_fields'] = ['optindate' => $value->DateSent, 'Agent ID' => $value->user, 'Datasource' => $SourceID];
                        $dataCheck['PREM_OPTIN']['success'] ++;
                    } else {
                        $dataCheck['PREM_OPTIN']['duplicate'] ++;
                    }
        ...
        /* START O2 PREMIUM */
        $ConsumerData = DB::connection('MainDialer')->select("Select s.sale_id,c.lead_id, c.title, c.first_name, c.last_name, c.phone_number, c.email, c.address1, c.address2, c.city, c.postal_code,c.teamname,c.current_supplier, c.happy_with_supplier, c.in_contract,c.bb_switch_call,u.full_name,s.campaign_sold_on,s.sale_date,u.user,c.DateSent
                    from O2Script.customers c
                    INNER JOIN O2Script.sales s ON c.lead_id = s.lead_id
                    JOIN custom_view.list l on l.lead_id=c.lead_id
                    join custom_view.users u on c.fullname = u.full_name
                    where c.DateSent BETWEEN '".$start." 00:00:00' AND '".$end." 23:59:59' AND c.bb_switch_call = 'yes'
                    and s.campaign_sold_on IN (1307,1304,1308)
                    group By c.lead_id");
        ...
        if (!empty($ConsumerData) && count($ConsumerData)) {
            foreach ($ConsumerData as $key => $value) {
                
                switch ($value->campaign_sold_on) {
                    case 1307:
                        $SSID = 'CON_OPTIN';
                ...
                $Optins->token = '';
                $Optins->custom_fields_AdentID = @$value->user;
                $Optins->campaign_name = $value->campaign_sold_on;
                $Optins->UniqueCode = $uniCode;
                $Optins->custom_fields_optindate = $value->DateSent;
                $Optins->duplicate_status = $duplicate;
                ...
                if ($Optins->save()) {
                    if ($duplicate == 'no') {
                        ...
                        $postData[$Count]['custom_fields'] = ['optindate' => $value->DateSent, 'Agent ID' => $value->user, 'Datasource' => $SSID];
