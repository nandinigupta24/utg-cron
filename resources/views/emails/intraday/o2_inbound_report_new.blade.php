                            ...
                            <tr style="background:#136975;">
                             ...
                             <tr style="background:#136975;">
                                ...
                                for($i=10;$i<=20;$i++){
                                    $start = $i;
                                    $end = $i+1;
                                   $dataOutPut =  get_intraday_o2sales_offered_drop($date.' '.$start.':00:00',$date.' '.$end.':00:00',$key,15);
...
//                                   $totalAbondaned[] = @$dataOutPut['drop'];
                                ...
                                <td align="center">{{$saleData = get_o2sales_intraday_sale($date,$key)}}</td>
                ...
                <th colspan="25" class="text-center">
                            ...
                            <tr style="background:#136975;">
                             ...
                             <tr style="background:#136975;">
                                ...
                                for($i=10;$i<=20;$i++){
                                    $start = $i;
                                    $end = $i+1;
                                   $dataOutPut =  get_intraday_o2sales_offered_drop_all($date.' '.$start.':00:00',$date.' '.$end.':00:00', array_keys($campaignId),15);
                                ...
                                <?php }?>
                                <td align="center">{{$totalCalls = (!empty($totalCall) && count($totalCall) > 0) ? array_sum($totalCall) : 0}}</td>
                                <td align="center">{{$totalSale = get_o2sales_intraday_all_sale($date,array_keys($campaignId))}}</td>
                    ...
                    $imageSLA = get_image_path_chart('http://export.highcharts.com/'.$result,'SLA-sale'.date('Y-m-d-H-i-s'),'/IntradayReport/O2Inbound/');
                    ...
                    <!--<img src="<?php echo $imageSLA; ?>" download='AHT'/>-->
                                ...
                                <td colspan="4"> <table class="table table-striped" style="color:#000;">
                                        ...
                                        <tr>
                                            <td class="text-center">{{$totalSale = get_o2sales_intraday_all_sale($date,array_keys($campaignId))}}</td>
                    ...
                    $context = stream_context_create($options);
                    $result = file_get_contents($url, false, $context);
                    $imageSLA = get_image_path_chart('http://export.highcharts.com/'.$result,'AWHT-sale-'.date('Y-m-d-H-i-s'),'/IntradayReport/O2Inbound/');
                    ...
                    $context = stream_context_create($options);
                    $result = file_get_contents($url, false, $context);
                    $imageSLA = get_image_path_chart('http://export.highcharts.com/'.$result,'ITO-sale-'.date('Y-m-d-H-i-s'),'/IntradayReport/O2Inbound/');
